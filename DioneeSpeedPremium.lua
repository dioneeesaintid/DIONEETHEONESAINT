...
-- v4.2 Final - +1 Speed Keyboard Escape | Candy & Chocolate
...
local Window = Rayfield:CreateWindow({
   ...
   LoadingTitle = "DIONEE PREMIUM v4.2 Final",
...
local function detectMap()
    ...
    if mapName == "Unknown" then
        for _,v in ipairs(workspace:GetChildren()) do
            if v:IsA("Folder") and #v:GetChildren() > 10 then
                mapName = v.Name
                break
            end
        end
...
task.spawn(function()
    ...
    while true do
        ...
        if current < lastWins then rebirths = rebirths + 1 end
        lastWins = current
        sessionWins = current - startWins
        if sessionWins < 0 then sessionWins = current; startWins = 0 end
        local elapsed = (tick() - startTime) / 60
        local rate = elapsed > 0 and math.floor(sessionWins / elapsed * 10) / 10 or 0
        pcall(function()
            winCounterLabel:Set({
                Title = "Wins: "..current,
                Content = "Session: +"..sessionWins.." | Rate: "..rate.."/min | Rebirths: "..rebirths
            })
...
FarmTab:CreateToggle({
   ...
   Callback = function(v)
      getgenv().AutoWin = v
      -- auto-set farm start on first enable
...
FarmTab:CreateToggle({
   ...
   CurrentValue = false,
   Flag = "AutoFarmTP",
   Callback = function(v) getgenv().AutoFarmTP = v end,
})
FarmTab:CreateSlider({
...
FarmTab:CreateToggle({Name = "Auto Collect +1", CurrentValue = false, Flag = "AutoCollect", Callback = function(v) getgenv().AutoCollect = v end})
...
PlayerTab:CreateInput({
   ...
   PlaceholderText = "16 / 500 / 9999 / 999999",
   RemoveTextAfterFocusLost = false,
   Callback = function(v)
      local n = tonumber(v)
      if n and n >= 16 then
         getgenv().WalkSpeed = n
         Rayfield:Notify({Title = "Speed Set", Content = "WalkSpeed: "..n, Duration = 1.5})
      end
   end,
})
PlayerTab:CreateButton({Name = "Speed 16", Callback = function() getgenv().WalkSpeed = 16; speedSlider:Set(16) end})
PlayerTab:CreateButton({Name = "Speed 50", Callback = function() getgenv().WalkSpeed = 50; speedSlider:Set(50) end})
PlayerTab:CreateButton({Name = "Speed 100", Callback = function() getgenv().WalkSpeed = 100; speedSlider:Set(100) end})
PlayerTab:CreateButton({Name = "Speed 250", Callback = function() getgenv().WalkSpeed = 250; speedSlider:Set(250) end})
PlayerTab:CreateButton({Name = "Speed 500", Callback = function() getgenv().WalkSpeed = 500; speedSlider:Set(500) end})
PlayerTab:CreateButton({Name = "Speed 1000", Callback = function() getgenv().WalkSpeed = 1000 end})
PlayerTab:CreateButton({Name = "Speed 9999", Callback = function() getgenv().WalkSpeed = 9999 end})
...
PlayerTab:CreateButton({
   ...
   Callback = function()
      ...
      getgenv().JumpPower = 50
      speedSlider:Set(16)
      Rayfield:Notify({Title = "Reset", Content = "Speed reset to 16", Duration = 1.5})
   end,
})
            ...
            if hum and hrp then
                ...
                if getgenv().AntiFling then
                    for _,v in ipairs(char:GetDescendants()) do
                        if v:IsA("BodyVelocity") or v:IsA("BodyAngularVelocity") or v:IsA("RocketPropulsion") then v:Destroy() end
                    end
                ...
                if getgenv().GodMode and hum.Health < hum.MaxHealth * 0.9 then
                    hum.Health = hum.MaxHealth
                end
            end
        end)
...
SettingsTab:CreateParagraph({Title = "DIONEE PREMIUM v4.2 Final", Content = "Final Premium By DIONEE | Map Auto-Detect + Win TP Farm"})
...
task.spawn(function()
    while true do
        ...
        if not zone then
            if winRemote then pcall(function() winRemote:FireServer() end) end
...
task.spawn(function()
    while true do
        task.wait(0.8)
        if getgenv().AutoFarmTP then pcall(function() checkpointTpRemote:FireServer(getgenv().FarmCheckpoint, "wins") end) end
        if getgenv().AutoRebirthAtWins then
            pcall(function()
                local wins = getWins()
                if wins >= getgenv().RebirthWinsThreshold then rebirthRemote:FireServer() end
            end)
        elseif getgenv().AutoRebirth then
            pcall(function() rebirthRemote:FireServer() end)
        end
        if getgenv().AutoClaim then
            pcall(function() claimGiftRemote:FireServer() end)
            pcall(function() verifyGroupRemote:InvokeServer() end)
        end
        if getgenv().AutoBuyAura then pcall(function() buyAuraRemote:InvokeServer(getgenv().AuraName, "Wins") end) end
        if getgenv().AutoBuyTrail then pcall(function() buyTrailRemote:InvokeServer(getgenv().TrailName, "Wins") end) end
        if getgenv().AutoEquipBest and equipPetRemote then
            pcall(function() equipPetRemote:FireServer("Best") end)
            pcall(function() equipPetRemote:InvokeServer("Best") end)
        end
    end
end)
        ...
        if getgenv().AutoCollect and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
            ...
            for _,v in ipairs(workspace:GetDescendants()) do
                if v:IsA("TouchTransmitter") and v.Parent and v.Parent:IsA("BasePart") then
                    local p = v.Parent
                    if (p.Position - hrp.Position).Magnitude < 28 then
...
task.spawn(function()
    ...
    local mapName, count = refreshMapScan()
    Rayfield:Notify({
       Title = "DIONEE PREMIUM v4.2 Final",
